...
pub fn patch(allocator: std.mem.Allocator, args: [][:0]u8) void {
    ...
    const dest_path = std.fmt.allocPrint(allocator, "{s}.patched{s}", .{ rom_file_name_base, rom_file_ext }) catch fatal("could not allocate memory for destination path", .{});
    defer allocator.free(dest_path);
    dest_dir.copyFile(rom_path, dest_dir, dest_path, .{}) catch fatal("could not copy ROM file", .{});
    ...
    // open patch file
    const patch_file = std.fs.cwd().openFile(patch_path, .{ .mode = .read_write }) catch fatal("could not open patch file \x1b[1m{s}\x1b[0m", .{patch_path});
    const patch_reader_buf = allocator.alloc(u8, std.math.maxInt(u16)) catch
        fatal("could not allocate memory for patch file reader buffer", .{});
    ...
    const patch_file_len = patch_reader_core.getSize() catch
        fatal("could not get patch file size", .{});
    ...
    while (patch_reader_core.logicalPos() < patch_file_len - 3) {
        const record = patch_reader.takeStruct(IpsPatchRecord, .big) catch fatal("could not get IpsPatchRecord", .{});

        patched_rom_writer_core.seekTo(record.offset) catch fatal("could not seek patched ROM file to offset", .{});
        if (record.length > 0) {
            patch_reader.streamExact(patched_rom_writer, record.length) catch fatal("could not stream data from patch file to patched ROM file", .{});
        } else {
            const rle_length = patch_reader.takeInt(u16, .big) catch fatal("could not read RLE length", .{});
            const rle_byte = patch_reader.takeByte() catch fatal("could not read RLE byte", .{});
            for (0..rle_length) |_| {
